...
librerias <- c("GEOquery","Biobase","BiocGenerics","magrittr","tidyr","dplyr","stringr","limma","EnhancedVolcano", "purrr","foreach","tidyverse", "xlsx","Hmisc")
libraries(librerias)
```
...
write.xlsx2(GSMs_filtrados, getwd())
```
...
## Hay muchos GSMS que no tienen material suplementario disponible para la descarga, lo que genera un problema con el str_split para lograr generar el output
mis.path <- list()
output <- list()
for (i in 1:length(mis.targets)){
  for (j in 1:length(mis.targets[[i]]$geo_accession)){
    output <- append(output, rownames(getGEOSuppFiles(mis.targets[[i]]$geo_accession[j], makeDirectory = TRUE, baseDir =getwd(), fetch_files = FALSE )) %>% unlist())# %>% strsplit('" "') %>% unlist())
    output <- output %>% unlist()
  }
  mis.path[[i]] <- append(mis.path, output) %>% unlist()
  output <- list()
}
...
cn <- 0
for (i in mis.path){
  if (sum(grepl('CH2',mis.path)) != 0) {
    a <- matrix(data = i,ncol=2 ) %>% as.data.frame()
    colnames(a) <- c("CH1", "CH2")
    a <-a %>% mutate(file_pathCH1= ifelse( str_detect(`CH1`, "CH2"), .[["CH2"]], .[["CH1"]]  ) )%>% 
mutate(file_pathCH2= ifelse( str_detect(`CH2`, "CH1"), .[["CH1"]], .[["CH2"]]  ) ) %>% select(-c(CH1,CH2))
  }
  else {a <- data.frame(file_pathCH1= i )
    
  }
  cn <- cn + 1
  test[[cn]]<-bind_cols(test[cn],a)
}



####backup
#mis.path <- append(mis.path, rownames(getGEOSuppFiles(mis.targets[[i]]$geo_accession[j], makeDirectory = TRUE, baseDir =getwd(), fetch_files = TRUE )) %>% 
             strsplit('" "') %>% unlist())
##idea inconclusa
#for (i in length(mis.path)){
 # if (mis.path[[i]] %>% == mis.targets[[i]]$geo_accession){
  #  mis.targets[i]$file_path <- 
  #}
...
#mis.path[6]
#to_bind[4]
#´mis.gsm <- mis.gsm %>% unlist()
#tt <- data.frame(file_path=mis.gsm)

#append()
#mis.path[1,] %>% class
#mis.targets[[1]]$file_path[1] <- c('hola')
#saveRDS(mis.gsm,file= "test.rds")

#a <- getGEOSuppFiles(mis.targets[[1]]$geo_accession[1], makeDirectory = TRUE, baseDir =getwd(), fetch_files = TRUE )
#rownames(a) %>% strsplit('" "') %>% unlist() -> b
#mis.targets[[1]] %>% unite()
#mis.gsm[[1]] %>% select(matches('GSM')) %>% split(.,seq(nrow(.))) -> objeto[[1]]
#t <- mis.gsm[[1]] %>% select(matches('GSM'))
#a = 0
#for (i in 1:length(mis.targets)){
 # a= a+length(mis.targets[[i]]$geo_accession)
#}
#a
#mis.path[1,]
```
