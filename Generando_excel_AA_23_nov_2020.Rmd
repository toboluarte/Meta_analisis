...
un_estudio%>% pData %>% select(matches('source.*ch') | matches('geo')| matches('label')) %>% unite("Cy3",source_name_ch1) %>% unite("Cy5",source_name_ch2) %>%  mutate_at('Cy3', ~str_replace(.,'.*non-infected.*','control')) %>% mutate_at('Cy3', ~str_replace(.,'.*salmonis-infected.*','infected'))%>%
                             mutate_at('Cy5', ~str_replace(.,'.*non-infected.*','control'))%>% mutate_at('Cy5', ~str_replace(.,'.*salmonis-infected.*','infected')) -> `GSE6924-GPL2899`  
...
```{r}
##Sin dye swap, Recordar que se esta considerando el virus atenuado como "control" e infectados LPS 
list_con_eset_filtrados$GSE10272 -> un_estudio 
un_estudio %>% pData %>% select(matches('source.*ch') | matches('geo')| matches('label')) %>%  mutate_at('source_name_ch1', ~str_replace(.,'.*att.*','control')) %>%  mutate_at('source_name_ch2', ~str_replace(.,'.*att.*','control')) %>%  mutate_at('source_name_ch1', ~str_replace(.,'.*LPS.*','infected')) %>% mutate_at('source_name_ch2', ~str_replace(.,'.*LPS.*','infected')) %>% mutate_at('source_name_ch1', ~str_replace(.,'.*IHNV.*','infected')) %>% mutate_at('source_name_ch2', ~str_replace(.,'.*IHNV.*','infected')) %>% unite("Cy5", source_name_ch1,label_ch1, sep ="-") %>%unite("Cy3", source_name_ch2,label_ch2, sep ="-")  %>%  mutate_at('Cy5', ~str_replace(.,'.*control.*','control'))%>% mutate_at('Cy5', ~str_replace(.,'.*infected.*','infected')) %>% mutate_at('Cy3', ~str_replace(.,'.*control.*','control'))%>% mutate_at('Cy3', ~str_replace(.,'.*infected.*','infected')) -> GSE10272
```
...
un_estudio %>% pData %>% select(matches('source_name_ch*') | matches('geo')| matches('label')| matches('title')) %>% unite("Cy3", source_name_ch1,label_ch1) %>% unite("Cy5", source_name_ch2,label_ch2) %>% mutate_at("Cy3",~str_replace(.,'.*control.*', 'control')) %>% mutate_at("Cy5",~str_replace(.,'.*infected.*', 'infected')) -> GSE15328
```
...
un_estudio %>% pData %>% select(matches('source_name_ch*') | matches('geo')| matches('label')) %>% unite('Alexa 555', source_name_ch1,label_ch1) %>% unite("Alexa 647", source_name_ch2,label_ch2) %>% mutate_at('Alexa 555', ~str_replace(.,".*control.*","control"))%>% mutate_at('Alexa 555', ~str_replace(.,".*infected.*","infected")) %>%  mutate_at('Alexa 647', ~str_replace(.,".*reference.*","control")) -> GSE19630
```

```{r}
#single channel 
list_con_eset_filtrados$GSE20310 -> un_estudio 
un_estudio %>% pData %>% select(matches('source_name_ch*') | matches('geo')| matches('label')) %>% unite('Cy3', source_name_ch1,label_ch1) %>% mutate_at('Cy3', ~str_replace(.,".*S.th.*", "infected")) %>% mutate_at('Cy3', ~str_replace(.,".*PBS.*", "control")) -> GSE20310
```
...
un_estudio %>% pData %>% select(matches('source_name_ch*') | matches('geo')| matches('label')) %>%  unite("Cy3", source_name_ch1,label_ch1) %>% mutate_at("Cy3",~str_replace(.,'.*63d.*', 'infected')) %>% mutate_at("Cy3",~str_replace(.,'.*0h.*', 'control')) -> GSE28357 
...
un_estudio %>% pData %>% select(matches('source_name_ch*') | matches('geo')| matches('label')) %>% unite("Cyanine 3", source_name_ch1,label_ch1) %>% mutate_at("Cyanine 3",~str_replace(.,'.*mock.*', 'control')) %>% mutate_at("Cyanine 3",~str_replace(.,'.*infected.*', 'infected')) %>% select(matches('Cyanine 3')| matches('geo')) -> GSE26651
```
...
un_estudio %>% pData %>% select(matches('source_name_ch*') | matches('geo')| matches('label')) %>% unite("Cyc3",source_name_ch1,label_ch1) %>% mutate_at("Cyc3",~str_replace(.,'.*Control.*', 'control'))%>% mutate_at("Cyc3",~str_replace(.,'.*Mock.*', 'control')) %>%mutate_at("Cyc3",~str_replace(.,'.*Challenged.*', 'infected')) %>% select(matches('cy')|matches('geo')) -> GSE51839
```
...
```{r}
##Dye swap, no hay informacion de los canales 
list_con_eset_filtrados$GSE87920 -> un_estudio 
un_estudio %>% pData %>% select(matches('source_name_ch*') | matches('geo')| matches('label'))
```
