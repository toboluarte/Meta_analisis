...
identical(target_final$name.x, target_final$name.y)

```
###
```{r}
### pulir y corroborar que funciona (método de loopeo)
...
for (i in 1:6){
  for (j in 1:length(mis.targets[[i]]$geo_accession)){
    output <- append(output, getGEOSuppFiles(mis.targets[[i]]$geo_accession[j], makeDirectory = TRUE, baseDir =getwd(), fetch_files = T ) %>% rownames())
