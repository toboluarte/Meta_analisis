...
parsing_from_match_to_end <- function(file_path, empty_list){
  id <- str_extract(file_path, 'GSM\\d+')
   as.data.frame(fread(file = file_path,sep = '\t', fill= TRUE, skip = 'Block')) -> empty_list[id]
   

}
as.data.frame(fread(the_test,sep = '\t',skip = 'asd'))-> the_result
parsing_from_match_to_end(the_test, my.gpr.DFs)
