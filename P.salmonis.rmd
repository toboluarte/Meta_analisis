...
load("E-MTAB-685.eSet.r")
#load ("A-MEXP-2065.adf.txt") ### esto no sirve, creo que sólo se debe hacer el Parsing.
expr = assayDataElement(study,"G")

## aparentemente no hay nada que filtrar
pData(study)
...
for(i in 1:length(names(my.gpr.DFs))){
  ...
  for (j in 1:length(old.names)) {
      ...
      if(old.names[i] == str_detect("^F6[0-9]{2}.*Mean$")){
        old.names[i] -> rojo_forward
      
    }
      else if (old.names[i] == str_detect("^F5[0-9]{2}.*Mean$")) {
        old.names[i] -> verde_forward
      
    }
    
      else if(old.names[i]== str_detect("^B6[0-9]{2}.*Median$") ) {
        old.names[i] -> rojo_background
      }
    
      else if (old.names[i] == str_detect("^B5[0-9]{2}.*Median$")) {
      old.names[i] -> verde_background
      }
    color_vector <- c(rojo_forward, verde_forward, rojo_background, verde_background)
    }
  my.cols.new.names <- color_vector %>% 
    str_extract_all("(^F[0-9]{3}.*Mean$)|(^B[0-9]{3}.*Mean$)|(^F[0-9]{3}+.*Median$)|(^B[0-9]{3}.*Median$)") %>%
    unlist() %>%
...
colnames(target.ok)[value] <- c("FileName")
```

```{r}
    RG          <- read.maimages(mis.targets$43255 ,source = "generic", columns = list(R= rojo_forward ,G= verde_forward,Rb=rojo_background ,Gb=verde_background), annotation = c("ProbeName", "SequenceID"))
