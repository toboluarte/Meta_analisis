...
```{r}
    RG          <- read.maimages(target.ok$FileName ,source = "generic", columns = list(R= "rojo_forward" ,G= "verde_forward",Rb="rojo_background" ,Gb="verde_background"), annotation = c("ProbeName", "SequenceID"))
...
test <- getAE("E-MTAB-685", type = "full")
rawset <- ae2bioc(mageFiles = test) ## duplicate Row.names are not allowed.

AEset<-ArrayExpress("E-MTAB-685")
AEsetnorm<- rma(AEset)
...
targets <- readTargets()
parsing_from_match_to_end("~/Documentos/Meta_analisis_teleosteos/Meta_analisis/A-MEXP-2065.adf.txt")
